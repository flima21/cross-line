...
class OpenCv:
    ...
    def capture_movements(self,path_file: str = '../../ouput/', file_name: str = 'movie.avi'):
        try:
            if self.thread_initialized == False:
                return 
            
            if self.cap.isOpened():
                self.out = cv2.VideoWriter(f'teste_movie.avi',cv2.VideoWriter_fourcc(*'MJPG'),15.,(640,480))
